...
class TrackerTraits
{
 ...
 public:
  ...
  virtual void findCellSeeds(const int iteration);
  ...
  void updateTrackingParameters(const std::vector<RecoIteration>& recoPars);
...
inline const int4 TrackerTraits<NLayers>::getBinsRect(const int iteration, const int layerIndex, float phi, float maxdeltaphi, float z1, float z2, float maxdeltaz) const noexcept
{
  ...
  if (zRangeMax < -mRecoParams[iteration].params.LayerZ[layerIndex] ||
      zRangeMin > mRecoParams[iteration].params.LayerZ[layerIndex] || zRangeMin > zRangeMax) {
  ...
  return int4{o2::gpu::GPUCommonMath::Max(0, utils.getZBinIndex(layerIndex, zRangeMin)),
              ...
              o2::gpu::GPUCommonMath::Min(mRecoParams[iteration].params.ZBins - 1, utils.getZBinIndex(layerIndex, zRangeMax)),
