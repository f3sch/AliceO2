  ...
  const int maxCellNeighbours = 1e2)
{
  for (int iCurrentCellIndex = blockIdx.x * blockDim.x + threadIdx.x; iCurrentCellIndex < nCells; iCurrentCellIndex += blockDim.x * gridDim.x) {
    ...
    for (int iNextCell{nextLayerFirstCellIndex}; iNextCell < nextLayerLastCellIndex; ++iNextCell) {
      ...
      const auto& trkl11 = tracklets[layerIndex + 2][currentCellSeed.getSecondTrackletIndex()];
      if (!rofOverlaps.isCompatible(layerIndex, trkl00.rof[0], layerIndex + 2, trkl11.rof[1])) {
        continue;
      }
  ...
  const float nSigmaCut)
{
  ...
  for (int iCurrentTrackletIndex = blockIdx.x * blockDim.x + threadIdx.x; iCurrentTrackletIndex < nTrackletsCurrent; iCurrentTrackletIndex += blockDim.x * gridDim.x) {
    ...
    for (int iNextTrackletIndex{nextLayerFirstTrackletIndex}; iNextTrackletIndex < nextLayerLastTrackletIndex; ++iNextTrackletIndex) {
      ...
      if (!rofOverlaps.isCompatible(layer, currentTracklet.rof[0], layer + 1, nextTracklet.rof[1])) {
        continue;
      }
...
GPUg() void __launch_bounds__(256, 1) computeLayerTrackletsMultiROFKernel(
  ...
  const int iSlice,
  const typename ROFOverlapTable<NLayers>::View rofOverlaps,
  const typename ROFTimeSliceTable<NLayers>::View timeSlices,
  ...
  const float MSAngle = -42.f)
{
  ...
  const auto& slice = timeSlices.getSlice(layerIndex, iSlice);
  for (unsigned int iROF{blockIdx.x}; iROF < slice.getEntries(); iROF += gridDim.x) {
    const uint32_t pivotROF = iROF + slice.getFirstEntry();
    ...
    auto clustersCurrentLayer = getClustersOnLayer(pivotROF, layerIndex, ROFClusters, clusters);
    ...
    for (int currentClusterIndex = threadIdx.x; currentClusterIndex < clustersCurrentLayer.size(); currentClusterIndex += blockDim.x) {
      ...
      for (int iV{startVtx}; iV < endVtx; ++iV) {
        ...
        for (int32_t targetROF{rofOverlap.getFirstEntry()}; targetROF < rofOverlap.getEntriesBound(); ++targetROF) {
          auto clustersNextLayer = getClustersOnLayer(targetROF, layerIndex + 1, ROFClusters, clusters);
            ...
            for (int nextClusterIndex{firstRowClusterIndex}; nextClusterIndex < maxRowClusterIndex; ++nextClusterIndex) {
              ...
              if (deltaZ / sigmaZ < NSigmaCut && (deltaPhi < phiCut || o2::gpu::CAMath::Abs(deltaPhi - o2::constants::math::TwoPI) < phiCut)) {
                if constexpr (initRun) {
                  ...
                  new (tracklets[layerIndex] + trackletsLUT[layerIndex][currentSortedIndex] + storedTracklets) Tracklet{currentSortedIndex, nextSortedIndex, tanL, phi, (short)pivotROF, (short)targetROF};
                }
                ++storedTracklets;
              }
            }
...
template void countTrackletsInROFsHandler<7>(const IndexTableUtils<7>* utils,
                                             ...
                                             const ROFTimeSliceTable<7>::View& timeSlices,
...
template void countCellsHandler<7>(const Cluster** sortedClusters,
                                   ...
                                   int** cellsLUTsArrayDevice,
                                   int* cellsLUTsHost,
                                   const typename ROFOverlapTable<7>::View rofOverlaps,
                                   const float bz,
                                   const float maxChi2ClusterAttachment,
                                   const float cellDeltaTanLambdaSigma,
                                   const float nSigmaCut,
