...
void TimeFrameGPU<NLayers>::loadROFOverlapTable()
{
  ...
  allocMem(reinterpret_cast<void**>(&d_flatTable), flatTableSize * sizeof(TableEntry), this->hasExternalDeviceAllocator());
  GPUChkErrS(cudaMemcpy(d_flatTable, hostView.mFlatTable, flatTableSize * sizeof(TableEntry), cudaMemcpyHostToDevice));
  allocMem(reinterpret_cast<void**>(&d_indices), hostTable.getIndicesSize() * sizeof(TableIndex), this->hasExternalDeviceAllocator());
  GPUChkErrS(cudaMemcpy(d_indices, hostView.mIndices, hostTable.getIndicesSize() * sizeof(TableIndex), cudaMemcpyHostToDevice));
  allocMem(reinterpret_cast<void**>(&d_layers), NLayers * sizeof(LayerTiming), this->hasExternalDeviceAllocator());
...
template <int NLayers>
void TimeFrameGPU<NLayers>::loadROFTimeSliceTable()
{
  GPUTimer timer("initialising device view of ROFTimeSliceTable");
  const auto& hostTable = this->getROFTimeSliceTable();
  const auto& hostView = this->getROFTimeSliceTableView();
  using ROFRange = ROFTimeSliceTable<NLayers>::ROFRange;
  ROFRange* d_flatTable{nullptr};
  uint8_t* d_flatMask{nullptr};
  int32_t* d_layerROFOffset{nullptr};
  size_t flatTableSize = hostTable.getFlatTableSize();
  allocMem(reinterpret_cast<void**>(&d_flatTable), flatTableSize * sizeof(ROFRange), this->hasExternalDeviceAllocator());
  GPUChkErrS(cudaMemcpy(d_flatTable, hostView.mFlatTable, flatTableSize * sizeof(ROFRange), cudaMemcpyHostToDevice));
  allocMem(reinterpret_cast<void**>(&d_flatMask), hostTable.getFlatMaskSize() * sizeof(uint8_t), this->hasExternalDeviceAllocator());
  GPUChkErrS(cudaMemcpy(d_flatMask, hostView.mFlatMask, hostTable.getFlatMaskSize() * sizeof(uint8_t), cudaMemcpyHostToDevice));
  allocMem(reinterpret_cast<void**>(&d_layerROFOffset), NLayers * sizeof(int32_t), this->hasExternalDeviceAllocator());
  cudaMemcpy(d_layerROFOffset, hostView.mLayerROFOffsets, NLayers * sizeof(int32_t),
             cudaMemcpyHostToDevice);
  mDeviceROFTimeSliceTableView = hostTable.getDeviceView(d_flatTable, d_flatMask, d_layerROFOffset);
}
...
{
  const auto& hostTable = this->getROFVertexLookupTable();
  if (!iteration && hostTable.needsUpdate()) {
