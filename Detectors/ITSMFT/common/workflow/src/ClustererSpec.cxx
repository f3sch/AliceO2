...
void ClustererDPL<N>::run(ProcessingContext& pc)
{
  ...
  for (uint32_t iLayer = 0; iLayer < ((DPLAlpideParam<N>::supportsStaggering()) ? NLayers : 1); ++iLayer) {
    int layer = (DPLAlpideParam<N>::supportsStaggering()) ? iLayer : -1;
    sw.Start();
    LOG(info) << mDetName << "Clusterer:" << iLayer << " pulled " << digits[iLayer].size() << " digits, in " << rofs[iLayer].size() << " RO frames";
    ...
    if (mUseMC) {
      ...
      LOG(info) << mDetName << "Clusterer:" << iLayer << " pulled " << labels.getNElements() << " labels ";
    ...
    sw.Stop();
    LOG(info) << mDetName << "Clusterer:" << iLayer << " pushed " << clusCompVec.size() << " clusters, in " << nROFs << " RO frames in " << sw.RealTime() << " s";
  }
  if constexpr (!DPLAlpideParam<N>::supportsStaggering()) { // need to send dummy data for the rest
    for (uint32_t iLayer = 1; iLayer < NLayers; ++iLayer) {
      pc.outputs().make<std::vector<o2::itsmft::CompClusterExt>>(Output{Origin, "COMPCLUSTERS", iLayer});
      pc.outputs().make<std::vector<o2::itsmft::ROFRecord>>(Output{Origin, "CLUSTERSROF", iLayer});
      pc.outputs().make<std::vector<unsigned char>>(Output{Origin, "PATTERNS", iLayer});
      if (mUseMC) {
        auto& sl = pc.outputs().make<o2::dataformats::MCTruthContainer<o2::MCCompLabel>>(Output{Origin, "CLUSTERSMCTR", iLayer});
        pc.outputs().make<std::vector<o2::itsmft::MC2ROFRecord>>(Output{Origin, "CLUSTERSMC2ROF", iLayer});
      }
    }
  }
...
DataProcessorSpec getClustererSpec(bool useMC)
{
  ...
  for (uint32_t iLayer = 0; iLayer < o2::itsmft::DPLAlpideParam<N>::getNLayers(); ++iLayer) {
