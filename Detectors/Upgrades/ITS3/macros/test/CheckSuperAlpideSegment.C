...
bool isTestDetectorToLocal(0);
bool isTestFlatToCurved(0);
bool isTestLocalToGlobal(0);

void CheckSuperAlpideSegment(bool isTestDetectorToLocal = 0,
                             bool isTestFlatToCurved = 0,
                             bool isTestLocalToGlobal = 0,
                             std::string inputGeom = "o2sim_geometry.root")
{
  ...
  if (isTestDetectorToLocal || isTestFlatToCurved) {
    ...
    int nPoint = 0;
    for (Int_t i = 0; i < mNRows; ++i) {
      for (Int_t j = 0; j < mNCols; ++j) {
        float xLocal = -1;
        float zLocal = -1;
        int row_trans = -1;
        int col_trans = -1;
            ...
            for (unsigned int iTile{0}; iTile < 12; ++iTile) {
              ...
              for (int row = 0; row < mNRows; row++) {
                for (int col = 0; col < mNCols; col++) {
                  ...
                  seg.curvedToFlat(xCurved, yCurved, xLocal_translate, yLocal_translate);
