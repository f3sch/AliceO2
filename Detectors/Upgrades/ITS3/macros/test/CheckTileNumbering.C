...
void CheckTileNumbering(const std::string& inputGeom = "o2sim_geometry.root", const std::string& deadmap = "")
{
  ...
  float phiOffsetL0 = std::asinf(o2::its3::constants::equatorialGap / 2.f / o2::its3::constants::radii[0]);
  float phiOffsetL1 = std::asinf(o2::its3::constants::equatorialGap / 2.f / o2::its3::constants::radii[1]);
  float phiOffsetL2 = std::asinf(o2::its3::constants::equatorialGap / 2.f / o2::its3::constants::radii[2]);
  ...
  for (unsigned int iDet{0}; iDet <= o2::its3::constants::detID::l2IDEnd; ++iDet) {
    ...
    float phi = std::atan2f(gloC.Y(), gloC.X());
  ...
  c = new TCanvas("cL1", "Numbering Layer 1", 4000, 3000);
  c->Divide(1, 2);
  c->cd(1);
  ...
  c = new TCanvas("cL2", "Numbering Layer 2", 4000, 3000);
  c->Divide(1, 2);
  c->cd(1);
