 ...
 private:
  ...
  struct WorkingHit {
    ...
    void recalculateIdeal(float phi, float z)
    {
      ...
      mPointDef.SetZ(z);
      LOGP(info, "Original Hit: ({},{},{})  --> ({},{},{})", mPoint.X(), mPoint.Y(), mPoint.Z(), mPointDef.X(), mPointDef.Y(), mPointDef.Z());
    }
  ...
  std::tuple<double, double, double> getDeformation(double u, double v) const { return {0.0, 0.0, 0.0}; }
   ...
   private:
    double DoEval(const double* x) const override
    {
      ...
      double xideal = mRadius * std::cos(phi), yideal = mRadius * std::sin(phi),
             zideal = z;
      double u, v;
      auto [dx, dy, dz] = mMis->getDeformation(u, v);
